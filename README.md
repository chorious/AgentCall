...
当前版本 / Current version: `coding/report worker split`
...
v6.5.0 延续 v6.2 worker closure 主线，并把 AgentCall worker 类型收敛为两类：`coding` worker 独占目标 workspace 的实现写入，`report` worker 共享 workspace lease 且只能写 report/scratch。`read_only` route 参数和纯只读 worker 工作线已移除。冻结实现基线仍见 [v6.2 code plan](docs/v6.2-code-plan.md)。
...
python agentcall.py runtime-release --version 6.5.0
```
