...
v6.7.2 延续 v6.2 worker closure 主线：保留 `coding` / `report` 两类 worker，同时把安全锁错误码收敛为 Rust `ErrorCode` 枚举、让 SQLite store writer 可按本机 6 并发 fan-out、并把 prompt pending 提交从 Codex 正常路径前移到 daemon 自动自愈。本 patch 修复并发容量语义：MCP bridge 会把当前 Codex session/thread 作为 owner 传给 daemon，`per_owner_max_sessions=6` 才是启动硬闸门，其他 Codex session 的 worker 不再耗尽本 session 的额度。冻结实现基线仍见 [v6.2 code plan](docs/v6.2-code-plan.md)。
...
python agentcall.py runtime-release --version 6.7.2
```
