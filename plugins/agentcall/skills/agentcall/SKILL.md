...
4. Use `agentcall_session(name=...)` for compact worker state, why, `primary_action`, available/debug actions, report info, workspace projection, active policy block details, and fresh control token when available.
...
- Treat default PTY workers as bounded-write workers: write tools may use route `write_paths`, `report_path`, and the session scratch; Bash is monitored by folder heartbeat audit and is not a filesystem sandbox.
- `reference_paths` are read/context recommendations, not permission boundaries.
- If `attention_status` is `blocked_by_policy`, do not wait inside the patience window or resend the same prompt. Inspect `policy_block`; for `category=workspace_audit_changed_dir`, approve an expected folder with `agentcall_session_send(action="approve_changed_dir", dir=..., reason=...)`, otherwise adjust scope, request a blocker report, interrupt, or stop.
...
- If the worker enters `accepted_live`, follow the summary's `primary_action`; owner-bound summaries may include a ready-to-send stop token in `primary_action.args.control_token`.
