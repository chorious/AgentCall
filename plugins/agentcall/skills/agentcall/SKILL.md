...
3. Use `agentcall_route(objective=..., workspace=..., allowed_paths=...)` to start bounded PTY utility work.
4. Use `agentcall_session(name=...)` for compact worker state, why, next actions, report info, workspace projection, and fresh control token.
5. Use `agentcall_session_send` only for one of the returned next actions.
...
- When a session summary shows `prompt_pending`, `prompt_missing`, or `prompt_commit_unacknowledged`, do not queue natural-language input. Wait if `can_wait=true`; otherwise use the returned `next_actions`, usually `agentcall_session_send(action="submit_pending_prompt")`.
