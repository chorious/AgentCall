...
v4.1 是 plugin-provided MCP + PTY-first 版本：
...
AgentCall v4.1 不自动 kill 用户可见 PTY worker，不尝试恢复旧 Claude 进程，也不让 Python 成为 live state writer。它更像一个本地调度与观测底座：让 Codex 更可靠地看见、控制和验收 Claude Code 的工作，而不是替代 Claude Code 或替代人的最终判断。
